...
CREATE VIEW books_issues IF NOT EXIST AS 
...
INSERT INTO issuance (card_number, inventory_number, issue_date, expected_return_date, actual_return_date)
VALUES (3, 4, 12-09-2025, 10-10-2025, NULL);
...
WHERE r.name = 'Василий' AND r.surname = 'Кузнецов' AND b.name = 'Собачье сердце' AND  AND bk.booking_datetime >= CURRENT_TIMESTAMP - INTERVAL '3 days';
...
SELECT 
    ...
    STRING_AGG(bi.location, ', ') AS местоположения
FROM book b
JOIN author a ON b.author_id = a.id
JOIN book_instance bi ON b.id = bi.book_id
WHERE bi.status = 'в наличии'  -- не выданы
AND NOT EXISTS (
    -- Проверка активных броней на эту книгу
