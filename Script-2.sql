...
CREATE TABLE public.author ( --2
	id SERIAL PRIMARY KEY,
	surname VARCHAR(50) NOT NULL,
	name VARCHAR(50) NOT NULL
);

CREATE TABLE public.publishing_house ( --3
	id SERIAL PRIMARY KEY,
	name VARCHAR(50) NOT NULL UNIQUE,
	town VARCHAR(50) NOT NULL
);

CREATE TABLE public.book ( --4
	id SERIAL PRIMARY KEY, 
	name VARCHAR(50) NOT NULL,
	...
	year_of_publishing INTEGER NOT NULL CHECK (year_of_publishing <= EXTRACT(YEAR FROM CURRENT_DATE)),
...
CREATE TABLE public.reader ( --5
	...
	registration_daye DATE NOT NULL
); 
...
CREATE TABLE public.book_instance (--6
...
CREATE TABLE public.issuance ( --7
    ...
    CONSTRAINT chk_issuance_dates 
        CHECK (actual_return_date IS NULL OR actual_return_date <= expected_return_date::DATE) 
); 

CREATE TABLE public.booking ( --8
